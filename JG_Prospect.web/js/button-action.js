...
function saveCallLog(e) {
    ...
    ajaxExt({
        ...
        type: 'POST',
        data: '{mode:"' + e.mode + '",num:"' + e.num + '",dur:"' + e.dur + '",userId:' + userId + '}',
        showThrobber: false,
        success: function (data, msg) {
            sequenceScopePhone.GetPhoneCallLogList();
            if (installUserId != null) {
                LoadNotes($('#user-' + installUserId), installUserId, userId);
            }
...
// Auto Dialer Logic
function startAutoDiualer(sender) {
    var num = 1;
    currentCallingPhoneSeq = 0;
...
function callNextUser() {
    ...
    if (currentCallingUserSeq < salesUsers.length) {
        ...
        alert('List finished or not yet started.');
