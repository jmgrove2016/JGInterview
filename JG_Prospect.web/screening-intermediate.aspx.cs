...
namespace JG_Prospect
{
    public partial class screening_intermediate : System.Web.UI.Page
    {
        ...
        protected void btnSaveProfile_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                var isExist = InstallUserBLL.Instance.UserExists(Convert.ToInt32(JGSession.LoginUserID),txtEmail.Text.Trim(), hdnPhone.Value);

                if (isExist == 1)
                {
                    string s = "<script language=JavaScript>alert('Email already exists!');</script>";
                    Page.ClientScript.RegisterStartupScript(this.GetType(), "email", s);
                }
                else if (isExist == 2)
                {
                    string s = "<script language=JavaScript>alert('Phone already exists!');</script>";
                    Page.ClientScript.RegisterStartupScript(this.GetType(), "phone", s);
                }
                else
                {
                    UpdateUserData();
        ...
        private void LoadDropDownData()
        {
            ...
            DataSet dsCurrency = CurrencyBLL.Instance.GetAllCurrency();
        ...
        private void LoadUserData()
        {
            ...
            if (dsUserDetails != null && dsUserDetails.Tables.Count > 0 && dsUserDetails.Tables[0].Rows.Count > 0)
            {
                ...
                ListItem Currency = ddlCurrency.Items.FindByValue(drDetails["CurrencyId"].ToString());

                if (Currency != null)
                {
                    ddlCurrency.SelectedIndex = ddlCurrency.Items.IndexOf(Currency);
                }
                ...
                var phone = drDetails["Phone"].ToString();
                #region Phone number logic
                phone = phone.Trim('+').Trim();
                if (phone.Length > 10)
                {
                    var result = Regex.Match(phone, @"(.{10})\s*$");
        ...
        private void UpdateUserData()
        {
            ...
            objInstallUser.CurrencyId = Convert.ToInt32(ddlCurrency.SelectedItem.Value);
